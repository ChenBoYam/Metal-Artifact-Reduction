...
class Denoiser_DNN(nn.Module):
    def __init__(self, input_shape):
        super(Denoiser_DNN, self).__init__()
...
def diff(x,use_image_gradient ='G1'):
    if use_image_gradient == 'G1':
        ...
        g2 = x + nn.AvgPool2d(5, stride=1, padding=2)(x)
        g = torch.cat((g1, g2), 1)
    else:
        g = x
    return g
...
patients_id_list = [ item for item in os.listdir(CT_dir) if os.path.isdir(os.path.join(CT_dir, item))]
...
train_id_list, test_id_list = train_test_split(patients_id_list, test_size=0.20)

train_patient_info_noise, train_patient_info_clear, train_noise_num, train_clear_num = get_patient_info(CT_dir, OMA_dir, train_id_list)
test_patient_info_noise, test_patient_info_clear, test_noise_num, test_clear_num = get_patient_info(CT_dir, OMA_dir, test_id_list)
...
for epoch in range(num_epoch):
    ...
    for i, ((noise_img, noise_label,_,_,_), (clear_img, clear_label,_,_,_)) in enumerate(zip(train_noise_loader, train_clear_loader)):
    ...
    with torch.no_grad():
        for i, ((noise_img, noise_label,_,_,_), (clear_img, clear_label,_,_,name)) in enumerate(zip(test_noise_loader, test_clear_loader)):
            if  i == 4:
                ...
                fig.savefig("ep{:02d}.png".format(epoch),bbox_inches = 'tight',pad_inches = 0)
                plt.close(fig)
                print("saving...")
            elif i > 4:
                break
