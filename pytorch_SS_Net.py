...
class Discriminator(nn.Module):
    def __init__(self, input_shape):
        ...
        self.model = nn.Sequential(
            ...
            *discriminator_block(32, 64),                                 #[batchsize,  256,  8,  8]
            *discriminator_block(64, 128),                                 #[batchsize,  512,  4,  4]
        )
...
for idx in range(3):
        ...
        for i, ((noise_img, noise_cls, supervised, noise_label,_), (clear_img, clear_cls,_,_,_)) in enumerate(zip(train_noise_loader, train_clear_loader)):
            ...
            for li, (ni,s,nl) in enumerate(zip(noise_img, supervised, noise_label)):
                ...
                if s:
                    ...
                    g_i_GT = g_n_GT + Variable(nl[None]).cuda()
                    dnn_p_GT = Dnn(g_i_GT.detach())
        ...
        with torch.no_grad():
            ...
            for i, ((noise_img, _,_,noise_label,_), (clear_img,_,_,clear_label,_)) in enumerate(zip(test_noise_loader, test_clear_loader)):
                ...
                if  i == 0:                
                    ...
                    fig.savefig("cv{:02d}ep{:02d}.png".format(idx+1,epoch),bbox_inches = 'tight',pad_inches = 0)
                    plt.close(fig)
                    print("saving...")
    ...
    metric_list.to_csv('cv%d_Result.csv'%(idx+1))
